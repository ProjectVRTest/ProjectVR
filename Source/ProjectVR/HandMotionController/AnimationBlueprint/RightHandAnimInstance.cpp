...
void URightHandAnimInstance::NativeUpdateAnimation(float DeltaSeconds)
{
	...
	{
		//구한 액터를 ARightHandMotionController로 형변환해서 RightHandController에 저장한다.
		ARightHandMotionController* RightHandController = Cast<ARightHandMotionController>(Actor);

		if (RightHandController) //형변환에 성공하면
		{
			HandState = RightHandController->HandState; //RightHandController이 가지고 있는 HandState를 가져와서 HandState에 넣어준다.
